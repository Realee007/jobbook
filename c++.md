...
变量可以使用的范围是由变量的作用域决定。不同层次的变量的作用域，就好像大小不一的纸片。把它们堆叠起来，就会发生部分纸片被遮盖的情况。我们把这种变量作用域的遮盖情况称为**变量的可见性（Visibility）**。 
...
1. 初始化
   ...
   2. **局部static变量**，在函数中有效，第一次进入函数初始化并且只初始化依次，以后进入函数将沿用上次的值。
...
内联函数应该在头文件中定义，这一点不同于其他函数。 当然也可以在源文件中定义，但此时只有定义的那个源文件可以用它完成内联，其它源文件如果只是包含该头文件使用内联函数得到话会出现链接错误，即找不到函数的定义。
