...
使用`push_back()`将新元素插入于vector尾部，首先会检查是否还有备用空间，如果有就直接在备用空间上构造元素，并调整迭代器finish使的vector变大，如果没有备用空间，就需要进行2倍扩容得到新vector，然后把原vector的内容拷贝到新vector，最后析构并释放原vector.
...
```
	...
	`size_type` 是 `unsigned int`型的,也就是说, i=0时再执行--i则i=0xFFFFFFFF(4294967295).于是程序会崩溃.
...
STL中，hashtable是以开链法，具体是在一个很大的Buckets vecotr（桶向量），vector的每个元素维护一个链表。STL以质数来设计vector，bucket (list)的总容量就是bucket vector的大小。

- 插入操作：

  `insert_unique` 插入元素不允许重。

  `insert_equal`插入允许重复。

1. 首先得判断待插入元素是否需要重建表格。判断原则：拿元素个数（包括待插入的元素）和bucket vector的大小比较。如果前者大于后者，就重建表格。
2.   经过重建表格的判断后，会先找到带插入元素的bucket位置n（根据哈希函数计算元素的位置），如果bucket[n]被占用，则进入bucket所对应的链表找对应是否存在键值相同元素，如果存在则return（而允许重复的话，则将其插入到目标位置之后），不存在生成新节点，加到链表的第一个位置。

- 删除操作：

  `erase`

	由于整个hash table由vector和Linked_List组合而成，因此删除需注意内存释放问题。

	根据key在哈希函数得出bucket[n]位置，并从bucket list的头开始，一一比对每个元素的键值，直到找到并删除该节点。
...
##### 装填因子
	...
	装填因子（负载因子）： = 填入表中的元素个数 / 哈希表的长度。是哈希表装满程度的标志因子。

##### 平均查找长度

 1. 查找成功时的平均查找长度

    = 表中每个元素查找成功时的比较次数之和 / 表中元素个数； 
...
H(18) =5 ，但是此时address=5已经被11占用，所以需要往后挪一位到address=6的位置。由于还是被占用，于是挪至address=7.挪2次，查找次数为3.
...
* 对于连续内存的容器，比如vector，删除当前的iterator而不将后续元素的iterator进行调整，会出现迭代器失效。 解决办法：保存erase()的返回iterator.
  ...
  vector<int>::iterator it;
   for (it = v.begin(); it != v.end(); /*it++*/)
   {
       ...
       //导致迭代器失效的错误写法
       if(*it>2)
           v.erase(it);	//erase掉后 当前it已失效，后续再进行it++报错
        */
       //正确写法
       if(*it>2)
       	 it = v.erase(it);	//erase掉后,为保证顺序容器内存的连续，需要将erase所返回的下一个迭代器赋值当前it.
       else
           it++;
   }
  ```

* 对于非连续内存的容器，如map，不正确的erase也会出现迭代器失效.
  ...
  for (auto it = m.cbegin(); it != m.cend() /* not hoisted */; /* no increment */)
  {
    if (must_delete)
    {
       ...
       //使用erase(it++)，
       //因为iter2 = iter++是iter先移到指向下一个节点，而iter2还是指向当前的节点。
    }
    else
    {
      ++it;
...
下面我们来看看VLD是如何工作的。在VisualC++中内置工具CRT Debug Heap工具（C Run-Time Libraries），在使用Debug版本分配内存时，它会在内存块中记录分配该内存的文件名和行号。当程序退出时CRT会在main函数返回时做一些清理工作，此时检查调试堆内存，如果仍然有内存没释放，则一定存在内存泄漏问题。从这些没有被释放的内存块的头中可以得到文件名和行号。这种静态的方法可以检查出内存泄漏，但是不知道泄漏究竟是怎么发生的，也不知道该内存分配语句是如何被执行到的，想要了解这些必须对内存分配过程进行动态跟踪。VLD就是这样做的，在每次内存分配的时候记录其上下文，当程序退出时对检测到的内存泄漏查找其上下文信息，并转换成报告输出到Output中。
