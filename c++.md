...
使用adapter(适配器)，一般以deque为底部结构（list也可以），并封闭其前端开口，即形成了stack. (修改某物接口，形成另一风貌---adapter设计模式)
...
同样使用adapter,一般以deque为底部结构，并封闭其底端的出口，和前端的入口，即形成queue。
...
以任何次序将任何元素推入容器内，但取出时一定是优先权最高的元素开始取。
