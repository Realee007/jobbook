...
IP 地址 ::= {< 网络号 >, < 主机号 >}			（符号"::=表示"定义为"）

![ip-1-classfiy.png](https://github.com/Realee007/jobbook/blob/master/src/image/ip-1-classfiy.png?raw=true)

网络号前面为类别位，其中A类为0，B类为10，C类为110...... 

特殊的地址：

A类地址：网络号全为1，即127.0.0.1 指环回测试（用于本机进程之间的通信）； 网络号全为0，即0.0.0.1指本网络。

主机号全0，即表示该IP地址是单个网络的地址，主机号全1表示该网络上所有的主机。



### 2. 子网划分

- 为解决IP地址利用率低；路由表变太大；和两级IP地址不够灵活的问题。于是提出划分子网。

- 通过在主机号字段中拿一部分作为子网号，把两级 IP 地址划分为三级 IP 地址。注意，外部网络看不到子网的存在。

  IP 地址 ::= {< 网络号 >, < 子网号 >, < 主机号 >}

  ![subnet_1.png](https://github.com/Realee007/jobbook/blob/master/src/image/subnet_1.png?raw=true)



  ![subnet_2.png](https://github.com/Realee007/jobbook/blob/master/src/image/subnet_2.png?raw=true)

因为，从IP数据报的首部无法看出目的主机所连接的网络是否进行了子网络划分，所以要使用子网，必须配置子网掩码。一个 B 类地址的默认子网掩码为 255.255.0.0，如果 B 类地址的子网占两个比特，那么子网掩码为 11111111 11111111 11000000 00000000，也就是 255.255.192.0。

子网掩码4个字节，A类1个字节，B类2个字节，C类3个字节，所以：
...
>1.根据子网划分
>
>对B类网络135.41.0.0/16需要划分为20个子网。 由于2^4<20<2^5。所以子网位只需要5位主机位就可以划分为32个子网，满足20个子网。 B类默认的子网掩码是255.255.0.0,转换为二进制为11111111.11111111.00000000.00000000。现在子网又占用了5位主机位，根据的子网掩码的定义，划分子网后的子网掩码应该为11111111.11111111.11111000.00000000，转换为十进制应该为255.255.248.0。所以此时主机位还有11位，可以满足2^11-2=2046台主机分配。
>
>2.根据主机数划分
>
>依然以上述例子：如果子网只有200台主机，以上述划分会造成IP地址浪费，为了更有效利用资源。
>
>2^7<200<2^8，也就是说，在B类网络的16位主机位中，保留8位主机位，其它的16－8=8位当成子网位，可以将B类网络135. 41.0.0划分成256（2^8）个能容纳256－1－1=254台（去掉全0全1情况）主机的子网。所以子网掩码可为11111111.11111111.11111111.00000000，转换为十进制为255.255.255.0。
...
3开头：重定向。需要客户端采取进一步的操作才能完成请求。

​	301 ：永久重定向。被请求的资源已永久移动到新位置，并且将来任何对此资源的引用都应该使用本响应返回的若干个 URI 之一

​	302：临时重定向。

4开头：（客户端）请求错误。这类的状态码代表了客户端看起来可能发生了错误，妨碍了服务器的处理
...
1. 浏览器第一次访问"www.baidu.com"解析为IP地址，如果Host文件中有对应的IP地址就直接取，否则使用到DNS；

2. 得到IP地址后，浏览器就要与服务器建立一个TCP连接，并且在此之前需要将消息传到服务器上，需要使用IP协议、ARP协议、OSPF协议。
...
3. 建立好TCP连接后，就使用HTTP协议向服务器请求网页内容. 服务器会回复给PC一堆HTML形式的文本。

4. 浏览器解析这些HTML文本，进行渲染页面。
...
《UNIX网络编程卷1：套接字联网API》(第三版) 2010  --- W.Richard Steve ...